...
function updateShow(req,res){
    ...
    var update = req.body;
    if(update.confirmed){
        //  Show.findById(showId, (err, showFounded) => {
        //      if(err) return res.status(500).send({mens:'Errorrr'});
        //      if(!showFounded) return res.status(404).send({message: 'No se ha podido actualizar el CHow'});
            
        //      function dup(j){
        //          return j == update.confirmed;
        //      }
        //      if(showFounded.confirmed.find(dup) != undefined ){

        //                   return res.status(404).send({m:'El usuario ya está confirmado'});
        //      }else{
        //         showFounded.confirmed.push(update.confirmed)
        //         update.confirmed = showFounded.confirmed
